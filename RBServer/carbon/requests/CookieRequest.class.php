...
/**
 * An implementation of the Request class that expects $_COOKIE data fields.
 */
class CookieRequest extends Request {
    ...
    public function extract($fields = NULL) {
        if ($fields) {
            ...
            foreach ($fields as $field => $required) {
                $data[$field] = filter_input(INPUT_COOKIE, $field);
    ...
    /**
     * Extracts data from the $_COOKIE array, but uses a first level field to specialise
    ...
    public function extract2($fieldLevel1, $fields = NULL) {
        ...
        if(in_array('application/json', $contentTypes)){
            ...
            if ($fieldLevel1 && $fields) {
                ...
                if(isset($_COOKIE[$fieldLevel1])){
                    foreach ($fields as $field => $required) {
                        $data[$field] = isset($_COOKIE[$fieldLevel1][$field]) ? $_COOKIE[$fieldLevel1][$field] : NULL;
