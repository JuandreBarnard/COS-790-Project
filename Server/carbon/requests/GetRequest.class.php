...
/**
 * An implementation of the Request class that expects $_GET data fields.
 */
class GetRequest extends Request {

    /**
     * Extracts data from the $_GET array.
    ...
    public function extract($fields = NULL) {
        if ($fields) {
            ...
            foreach ($fields as $field => $required) {
                $data[$field] = filter_input(INPUT_GET, $field);
    ...
    /**
     * Extracts data from the $_GET array, but uses a first level field to specialise
    ...
    public function extract2($fieldLevel1, $fields = NULL) {
        ...
        if(in_array('application/json', $contentTypes)){
            ...
            if ($fieldLevel1 && $fields) {
                ...
                if(isset($_GET[$fieldLevel1])){
                    foreach ($fields as $field => $required) {
                        $data[$field] = isset($_GET[$fieldLevel1][$field]) ? $_GET[$fieldLevel1][$field] : NULL;
